...
my_prompt() {
  ...
  local PS_USER_COLOR=$([ "$LAST_CMD_EXIT_CODE" = "0" ] && echo "$CYAN" || echo "$RED");

  git rev-parse --git-dir > /dev/null 2>&1;
  local IS_WD_GIT_REPO="$?";

  local GIT_INFOS;
  local CURRENT_DIR;
  if [ "$IS_WD_GIT_REPO" != "0" ]; then
  ...
  local PS_USER="$PS_USER_COLOR\\u$MAGENTA@$PS_USER_COLOR\\H$MAGENTA:$RESET";
  local PS_CURRENT_DIR=$([ "$IS_WD_GIT_REPO" != "0" ] && echo "$YELLOW$CURRENT_DIR$RESET" || echo "${BLUE}git$MAGENTA:$YELLOW$CURRENT_DIR");
  local PS_GIT=$([ "$IS_WD_GIT_REPO" = "0" ] && echo " $GIT_INFOS" || echo "");
  local PS_SU="\$ > ";

  PS1="$PS_TITLEBAR$PS_CHROOT$PS_TIME$PS_USER$PS_CURRENT_DIR$PS_GIT$PS_SU";
}
